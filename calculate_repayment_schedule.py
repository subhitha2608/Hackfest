...
def create_repayment_schedule(loan_id):
    with engine.connect() as conn:
        # Execute SQL query to get loan details
        result = conn.execute(text("SELECT loanamount, interestrate, loanterm, startdate "
                                   "FROM loans WHERE loanid = :loan_id"), 
                              {"loan_id": loan_id})

        loan_amount = result.fetchone()[0]
        interest_rate = result.fetchone()[1]
        loan_term = result.fetchone()[2]
        start_date = result.fetchone()[3]
        ...
        while payment_number <= loan_term:
            ...
            conn.execute(text("INSERT INTO repaymentschedule (loanid, paymentnumber, paymentdate, principalamount, interestamount, totalpayment, balance)
                         ...
                         {"loan_id": loan_id, "payment_number": payment_number, "payment_date": payment_date, 
