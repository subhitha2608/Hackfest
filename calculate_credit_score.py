...
def calculate_credit_score(customer_id):
    ...
    query = text("""
        SELECT 
            COALESCE(ROUND(SUM(loan_amount), 2), 0), 
    ...
    query = text("""
        SELECT 
            COALESCE(ROUND(SUM(balance), 2), 0)
        FROM 
            credit_cards
        WHERE 
            customer_id = :customer_id
    ...
    query = text("""
        ...
        FROM 
            payments
        WHERE 
            customer_id = :customer_id AND status = 'Late'
    """)
    result = conn.execute(query, {'customer_id': customer_id})
    ...
    if v_credit_score < 500:
        query = text("""
            INSERT INTO 
                credit_score_alerts (customer_id, credit_score, created_at)
            VALUES 
                (:customer_id, :credit_score, NOW())
        """)
