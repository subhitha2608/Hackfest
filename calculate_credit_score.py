...
# Load the database configuration from config.py
...
def calculate_credit_score(p_customer_id):
    ...
    """)
    result = engine.execute(query, p_customer_id=p_customer_id)
    ...
    if total_loan_amount > 0:
        v_credit_score += (total_repayment / total_loan_amount) * 400  # 40% weight for loan repayment
    else:
        v_credit_score += 400  # If no loans, give average score for this factor

    if credit_card_balance > 0:
        v_credit_score += (1 - (credit_card_balance / 10000)) * 300  # 30% weight for credit card utilization
    else:
        v_credit_score += 300
